...
namespace Veterinaria__VIP_1._0_
{
    public partial class ServicioVeterinario : Form
    {
        ...
        public ServicioVeterinario(string user)
        {
            this.user = user;
            InitializeComponent();
        }
        ...
        {
            if (txtNM.Text == "" | textTAM.Text == "" | txtFC.Text == "" | txtHC.Text == "" | txtTP.Text == "")
            {
                MessageBox.Show("Uno o más campos están incompletos.");
            }
            else
            {
                if (cn.confCitaVet(user, txtNM.Text, textTAM.Text, txtFC.Text, txtHC.Text, txtTP.Text) == 1)
                {
                    MessageBox.Show("Registro de cita exitoso");
                    dataGridView1.DataSource = cn.conCitaMed(user);
        ...
        private void dataGridView1_SelectionChanged(object sender, EventArgs e)
        {
            ...
            textTAM.Text = row.Cells[1].Value.ToString();
            txtFC.Text = row.Cells[2].Value.ToString();
            txtHC.Text = row.Cells[3].Value.ToString();
            txtTP.Text = row.Cells[4].Value.ToString();
        }

        private void ServicioVeterinario_FormClosing(object sender, FormClosingEventArgs e)
        {
            Application.Exit();
        }
    }
